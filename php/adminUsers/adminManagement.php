...
try {
    ...
    if ($page_action === 'edit') {
        if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
            header("Location: admin.php?page=adminManagement&error=" . urlencode("Geçersiz yönetici ID'si belirtildi."));
            exit();
        }
        
        $user_id = (int)$_GET['id'];
        
        $sql_edit = "SELECT id, ad, soyad, email FROM adminusers WHERE id = :id";
        ...
        if (!$edit_user) {
            header("Location: admin.php?page=adminManagement&error=" . urlencode("Düzenlenecek yönetici bulunamadı."));
            exit();
        }
    }
    
    // LİSTELEME Modu: Tüm yöneticileri çek
    $sql_list = "SELECT id, ad, soyad, email FROM adminusers ORDER BY id ASC";
...
<link rel="stylesheet" href="../../css/adminUsers/adminManagement.css"> 
...
<?php if ($page_action !== 'add' && $page_action !== 'edit'): ?>
 <a href="admin.php?page=adminManagement&action=add" class="btn-add-admin">
...
<?php endif; ?>
 ...
 <div class="form-card">
 <h3>Yeni Yönetici Kaydı</h3>
    <form id="addAdminForm" action="../../controller/adminUsers/adminManagementController.php" method="POST">
     <input type="hidden" name="action" value="add_admin">

  <div class="form-group">
  <label for="ad">Adı <span style="color: red;">*</span></label>
  <input type="text" id="ad" name="ad" required placeholder="Yönetici Adı">
  </div>

  <div class="form-group">
  <label for="soyad">Soyadı <span style="color: red;">*</span></label>
  <input type="text" id="soyad" name="soyad" required placeholder="Yönetici Soyadı">
  </div>
  ...
  <button type="submit" class="btn-form-submit">
    <i class="fas fa-user-plus"></i> Yöneticiyi Ekle
  </button>
  <a href="admin.php?page=adminManagement" class="btn-form-cancel">İptal</a>
  </div>
 </form>
 </div>
...
<?php endif; ?>
 ...
 <h3><?php echo htmlspecialchars($edit_user['ad'] . ' ' . $edit_user['soyad']); ?> Bilgilerini Düzenle</h3><br>
      ...
      <form id="updateInfoForm" action="../../controller/adminUsers/adminManagementController.php" method="POST">
    <input type="hidden" name="action" value="update_admin_info">
    <input type="hidden" name="id" value="<?php echo $edit_user['id']; ?>">

   <div class="form-group">
   <label for="ad">Adı <span style="color: red;">*</span></label>
   <input type="text" id="ad" name="ad" required value="<?php echo htmlspecialchars($edit_user['ad']); ?>" placeholder="Yönetici Adı">
   </div>

   <div class="form-group">
   <label for="soyad">Soyadı <span style="color: red;">*</span></label>
   <input type="text" id="soyad" name="soyad" required value="<?php echo htmlspecialchars($edit_user['soyad']); ?>" placeholder="Yönetici Soyadı">
   </div>
   ...
   <a href="admin.php?page=adminManagement" class="btn-form-cancel">İptal</a>
      ...
      <form id="resetPassForm" action="../../controller/adminUsers/adminManagementController.php" method="POST">
...
<?php if (empty($users)): ?>
 ...
 Kayıtlı yönetici bulunmamaktadır.
 </div>
<?php else: ?>
 ...
 <table class="admin-table">
  ...
  <tr>
   ...
   <td><?php echo htmlspecialchars($user['ad']) . ' ' . htmlspecialchars($user['soyad']); ?></td> 
   <td><?php echo htmlspecialchars($user['email']); ?></td>
   <td style="text-align: center;">
             <a href="admin.php?page=adminManagement&action=edit&id=<?php echo $user['id']; ?>" class="action-btn btn-edit">
...
<script src="../../js/adminUsers/adminManagement.js"></script>
</body>
</html>